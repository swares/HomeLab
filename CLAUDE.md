...
runs Vault; RPi 4B runs Pi-hole (192.168.1.116); secondary DNS is opi-zero2w-1 (192.168.1.184); Home Assistant runs as a k3s Deployment in the `home-assistant` namespace; lldap runs as a k3s Deployment in the `lldap` namespace (ldap-1 VM decommissioned
